...
/**
 * Bancontact payment gateway for Belgium.
 *
 * @since 1.0.0
 */
class BancontactGateway extends AbstractRegionalGateway {
	 ...
	 * @var string
	 */
	protected string $id = 'bancontact';
	 ...
	 * @var array
	 */
	protected array $countries = array( 'BE' );
	...
	public function process_payment( int $booking_id, array $payment_data ): array {
		...
		// Create Bancontact payment via Stripe.
		...
		$response = wp_remote_post(
			...
			array(
				...
				'body'    => array(
					...
					'confirm'                => 'true',
					'payment_method_data[type]' => 'bancontact',
		...
		update_post_meta( $booking_id, '_bkx_payment_method', 'bancontact' );
		update_post_meta( $booking_id, '_bkx_payment_status', 'pending' );

		$this->log_transaction( $booking_id, 'bancontact_initiated', array(
			'payment_id' => $body['id'],
		) );
		...
		return array(
			...
			'message'      => __( 'Redirecting to Bancontact...', 'bkx-regional-payments' ),
		);
	}
	...
	public function process_refund( int $booking_id, float $amount, string $reason = '' ): array {
		...
		if ( ! $payment_id ) {
			return array(
				'success' => false,
				'message' => __( 'No Bancontact transaction found.', 'bkx-regional-payments' ),
			);
		}
	...
	public function render_payment_fields(): void {
		?>
		<div class="bkx-bancontact-fields">
			<p class="bkx-payment-description">
				<?php esc_html_e( 'Pay securely with Bancontact. You will be redirected to complete the payment.', 'bkx-regional-payments' ); ?>
			</p>
			<div class="bkx-bancontact-logo">
				<img src="<?php echo esc_url( BKX_REGIONAL_PAYMENTS_URL . 'assets/images/bancontact.svg' ); ?>"
					 alt="Bancontact" style="max-width: 100px;">
			</div>
		</div>
		<?php
	}
