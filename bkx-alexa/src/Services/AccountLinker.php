<?php
/**
 * Account Linker for Amazon Alexa.
 *
 * Handles OAuth 2.0 account linking between Amazon and WordPress.
 *
...
class AccountLinker {
	...
	/**
	 * Handle authorization request from Alexa.
	 *
	...
	public function handle_auth_request( $request ) {
		...
		set_transient( 'bkx_alexa_auth_state_' . $state, array(
		...
		if ( is_user_logged_in() ) {
			include BKX_ALEXA_PATH . 'templates/auth/consent.php';
			exit;
		}
	...
	private function exchange_code( $code ) {
		$code_data = get_transient( 'bkx_alexa_auth_code_' . $code );

		if ( ! $code_data ) {
			return new \WP_Error( 'invalid_code', 'Invalid or expired authorization code', array( 'status' => 400 ) );
		}

		delete_transient( 'bkx_alexa_auth_code_' . $code );
	...
	private function refresh_token( $refresh_token ) {
		...
		$account = $wpdb->get_row(
			$wpdb->prepare(
				"SELECT * FROM {$wpdb->prefix}bkx_alexa_accounts WHERE refresh_token = %s", // phpcs:ignore
				$refresh_token
			)
		);
	...
	private function store_tokens( $user_id, $access_token, $refresh_token, $expires_in ) {
		...
		$existing = $wpdb->get_var(
			$wpdb->prepare(
				"SELECT id FROM {$wpdb->prefix}bkx_alexa_accounts WHERE wp_user_id = %d", // phpcs:ignore
				$user_id
			)
		);
		...
		if ( $existing ) {
			...
			$data['amazon_user_id'] = 'pending_' . $user_id;
	...
	public function get_user_by_token( $access_token ) {
		...
		$account = $wpdb->get_row(
			$wpdb->prepare(
				"SELECT * FROM {$wpdb->prefix}bkx_alexa_accounts WHERE access_token = %s AND token_expires > %s", // phpcs:ignore
				$access_token,
	...
	/**
	 * Link Amazon user ID to account.
	 *
	 * @param string $amazon_user_id Amazon user ID.
	 * @param int    $wp_user_id     WordPress user ID.
	 * @return bool
	 */
	public function link_amazon_user( $amazon_user_id, $wp_user_id ) {
		global $wpdb;

		return $wpdb->update(
			$wpdb->prefix . 'bkx_alexa_accounts',
			array( 'amazon_user_id' => $amazon_user_id ),
	...
	/**
	 * Get account by Amazon user ID.
	 *
	 * @param string $amazon_user_id Amazon user ID.
	 * @return object|null
	 */
	public function get_by_amazon_user( $amazon_user_id ) {
		global $wpdb;

		return $wpdb->get_row(
			$wpdb->prepare(
				"SELECT * FROM {$wpdb->prefix}bkx_alexa_accounts WHERE amazon_user_id = %s", // phpcs:ignore
				$amazon_user_id
			)
		);
	}
	...
	public function get_linked_count() {
		...
		return (int) $wpdb->get_var(
			"SELECT COUNT(*) FROM {$wpdb->prefix}bkx_alexa_accounts" // phpcs:ignore
		);
	}
